...
初回起動時は「使い方」モーダルが自動で開き、基本の流れとキー操作を確認できます。
2回目以降も右上の「**使い方**」ボタン、または <kbd>?</kbd> キーでいつでも開けます。

![初回に表示される使い方モーダル。4ステップの流れとキーボード一覧](images/screenshot-help.png)

### ガイドツアー

モーダルの「**ガイドツアーで始める**」を押すと、実際の画面上で「次に押すボタン」を
スポットライトと吹き出しで順番に案内します（デモ音源の読み込み → 再生 → 打刻 →
微調整 → 書き出し、の5ステップ）。実際に操作すると自動で次のステップへ進みます。

![ガイドツアー。デモ音源ボタンがハイライトされ、吹き出しで操作を案内している](images/screenshot-tour.png)

- <kbd>Esc</kbd> または吹き出しの「終了」でいつでも中断できます。
...
（チャート雛形・プレビュー）が存在しないこと、使い方モーダルの初回自動表示 / `Esc` / `?`、
ガイドツアーの開始〜完了（デモ音源 → 再生 → 打刻 → 微調整 → 保存、中断と再開を含む）、
再訪問時の非表示。
