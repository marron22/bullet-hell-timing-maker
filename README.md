...
1. **動画/音声を選択** … 動画または音声ファイルを読み込む（手元に無ければ「デモ音源」でも進めます）。
...
1. **動画/音声を読み込む** … 「動画/音声を選択」ボタン、またはタイムラインへ動画/音声ファイルをドロップ。動画は画面内プレビューで確認できます。
...
- **動画/音声ファイル自体は保存されません。** 復元後は音源を読み込み直してください。
- 別ステージ／別 PC へ引き継ぐときは「作業内容を保存」の JSON で書き出して共有します。

## danmaku-lab プレビュー（任意）

ヘッダーの「**プレビュー**」トグルで、右側に danmaku-lab の弾幕プレビューを開けます。
再生・スクラブ・マーカーのダブルクリックなど再生ヘッドの変化に合わせて、プレビュー側の
時計も追従します（音声は timing-maker 側のみで再生されます）。

- danmaku-lab サーバー（`serve.ps1`、既定ポート 8090）が起動している必要があります。
  未起動の場合はその旨を案内表示します（自動起動はしません）。
- URL（既定 `http://localhost:8090/Tools/danmaku-lab/index.html?embed=1`）とステージ名は
  入力欄で変更でき、`localStorage` に記憶されます。
- **timing-maker 自体を `file://` で開いている場合、danmaku-lab 側の origin 検証により
  シークが反映されないことがあります**（`http://localhost` 等のサーバー経由で開くと解消します）。
- このプレビューは補助表示であり、既存の書き出し（作業内容 JSON / AI用指示 Markdown）の
  形式には一切影響しません。
